...
/**
 * Class Timer controls the execution of a {@link TimerTask
 * </CODE>TimerTask<CODE>}'s timed actions.
 * <P>
 ...
 * </FONT>
 * <P>
 * When a timer is created, it is associated with a {@link TimerTask
 * </CODE>TimerTask<CODE>}. The timer is initially <B>stopped.</B> The timer can
 * then be <B>started</B> in various ways. The timer will then <B>time out</B>
 * at some instant, depending on how it was started. When the timer times out,
 * the timer becomes <B>triggered,</B> and the timer's timer task's
 * <TT>action()</TT> method is called. The timer task can stop the timer, or it
 ...
 * timer was started with a <B>periodic timeout,</B> the timer automatically
 * starts itself to time out again after a certain interval. If the timer was
 * started with a <B>one-shot timeout,</B> the timer automatically stops itself.
 * <P>
 ...
 * by calling a {@link TimerThread </CODE>TimerThread<CODE>}'s
 * <TT>createTimer()</TT> method, giving a timer task to associate with the
 ...
 * timers to call the timer tasks' <TT>action()</TT> methods at the proper
 ...
 * timeout occurred.
 * <P>
 * Classes Timer, {@link TimerTask </CODE>TimerTask<CODE>}, and {@link
 * TimerThread </CODE>TimerThread<CODE>} provide capabilities similar to classes
...
// Hidden data members.
	...
	// Predecessor and successor timers in the timer thread's linked list.
	Timer myPredecessor;
	Timer mySuccessor;
	...
	int myState = STOPPED;
		...
		static final int TRIGGERED = 2;

	// Kind of timeout.
	int myKind;
		static final int ONE_SHOT_TIMEOUT       = 0;
		static final int FIXED_RATE_TIMEOUT     = 1;
		static final int FIXED_INTERVAL_TIMEOUT = 2;
...
// Hidden constructors.
	...
	/**
	 * Construct a new timer with the given timer thread and timer task.
	 *
