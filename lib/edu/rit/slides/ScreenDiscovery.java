//******************************************************************************
//
// File:    ScreenDiscovery.java
// Package: edu.rit.slides
// Unit:    Interface edu.rit.slides.ScreenDiscovery
//
...
/**
 * Interface ScreenDiscovery is the remote interface for an exported screen
 * discovery object in the Slides application. A screen discovery object is used
 * to discover groups of screen objects.
 * <P>
 ...
 * interface {@link DiscoverableScreen </CODE>DiscoverableScreen<CODE>} in
 * addition to interface {@link Screen </CODE>Screen<CODE>}. To find screen
 ...
 * <TT>report()</TT> method. Interface {@link DiscoverableScreen
 * </CODE>DiscoverableScreen<CODE>} extends interface ScreenDiscovery and
 * defines an additional <TT>request()</TT> method.
 * <P>
 * Each discoverable screen object repeatedly invokes the <TT>report()</TT>
 * method on an omnihandle for interface ScreenDiscovery, passing in the
 * theatre's name and the theatre's multihandle for the theatre to which the
 * screen object belongs. Executing these <TT>report()</TT> method calls, all
 * the screen discovery objects build up a list of the theatres that are out
 * there.
 * <P>
 ...
 * calls (since interface {@link DiscoverableScreen
 * </CODE>DiscoverableScreen<CODE>} extends interface ScreenDiscovery). Why? If
 * a discoverable screen object receives a <TT>report()</TT> method call <I>for
 * its own theatre</I> -- that is, a <TT>report()</TT> method call performed by
 * some other member of its own theatre -- the discoverable screen object
 * reschedules its own next <TT>report()</TT> method call for a new randomly
 * chosen interval. In this way only one theatre member at a time calls
 * <TT>report()</TT>, reducing the network traffic.
 * <P>
 * When a new device arrives and wants to find out which theatres are out there,
 * the device can call <TT>request()</TT> on an omnihandle for interface {@link
 * DiscoverableScreen </CODE>DiscoverableScreen<CODE>}. In response, each
 * discoverable screen object reschedules its next <TT>report()</TT> method call
 * for one-tenth the usual interval -- that is, a random interval in the range
 * (0.02 <I>leasetime</I>) to (0.04 <I>leasetime</I>) (for example, 0.6 to 1.2
